...
```
4. Finally, we need to copy the bash script in our path and execute it:
```
sudo ln -sf ./HackTheBox.sh /usr/bin/HackTheBox
HackTheBox <Target IP>
```
> If you already have the keymap set, you can set anotherone in keys.lua and change the script function "set_bar" with `xdotool key 'your keymap' &>/dev/null`
